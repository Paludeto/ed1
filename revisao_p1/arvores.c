...
// Calculo de numero de elementos
int totalElementos(NO_ARVORE **no) {

    if ((*no) == NULL) {
        return 0;
    }

    int total = 1 + totalElementos(&(*no)->esq) +  totalElementos(&(*no)->dir);

}

// Conta o numero de nos nao-folha de uma ABB
int contaNaoFolha(NO_ARVORE **no) {

    if (*(no) == NULL || (*no)->esq == NULL && (*no)->dir == NULL) {
        return 0;
    }

    return 1 + contaNaoFolha(&(*no)->esq) + contaNaoFolha(&(*no)->dir);

}

// Calculo de altura da arvore
int calculaAlturaArvore(NO_ARVORE **no) {

    if ((*no) == NULL) {
        return -1;
    }

    int hDir = 1 + calculaAlturaArvore(&(*no)->dir);
    int hEsq = 1 + calculaAlturaArvore(&(*no)->esq);
...
bool checaBst(NO_ARVORE **no) {
    ...
    return checaBst(&(*no)->dir) && checaBst(&(*no)->esq);
...
int main(void){
    ...
    int total = totalElementos(&raiz);

    printf("Num total elementos: %d\n", total);

    total = contaNaoFolha(&raiz);

    printf("Total de nos nao-folha: %d\n", total);

    total = calculaAlturaArvore(&raiz);

    printf("Altura da arvore: %d\n", total);

    bool ehBst = checaBst(&raiz);
